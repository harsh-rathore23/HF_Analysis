...
double dR(const reco::Candidate* a1, const reco::GenParticle* a2){
	...
	double phi1=a1->phi();
	double phi2=a2->phi();
	double x = sqrt(pow((eta1-eta2),2)+pow((phi1-phi2),2));
	return x;
}
...
/////////////////Jets///////////////////////////////
      ...
      edm::Handle<reco::PFJetCollection> recjhandle;
...
//      edm::Handle<edm::ValueMap<bool> > tight_id_decisions;
      ...
      edm::EDGetTokenT<reco::PFJetCollection> recjtoken_;
...
ZEE_RecHit_NTuplizer::ZEE_RecHit_NTuplizer(const edm::ParameterSet& iConfig):
   ...
   xtoken_(consumes<GenEventInfoProduct>(edm::InputTag("generator"))),
	gpctoken_(consumes<reco::GenParticleCollection>(edm::InputTag("genParticles"))),
        gjctoken_(consumes<reco::GenJetCollection>(edm::InputTag("ak4GenJets"))),
        recjtoken_(consumes<reco::PFJetCollection>(edm::InputTag("ak4PFJets")))
{   
...
/////////////Code block: RecoJets/////////////////////
        for(unsigned int q = 0; q< recjhandle->size(); ++q){
                ...
                const vector<reco::PFCandidatePtr>& rjconst= pfj->getPFConstituents();
...
/*string ZEE_RecHit_NTuplizer::fun(){cout<<"/n why /n"<<endl;
                return "message";
             }*/
